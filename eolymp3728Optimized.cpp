...
bool ans[MaxK];
   ...
   int Parent[MaxN];
         inline void make_one_element_set (int v) {
    	                                        Parent[v] = v;
         }
    ...
    inline void union_sets_of(int v1, int v2) {
...
int main(){
  ...
  int u, v;
   for (int i=0; i<m; i++) {
       scanf("%d %d",&u, &v); //cin >> u >> v; //dumb input of (u,v) because we will restore whole net from cuts!
   }
  string a; // a may be "cut" else "ans"
  char op[4];  
  // op may be "cut" else "ans"
  int L=0, T; // L is number of answers, T is the type of query: 0=cut 1=ans 
    for (int j=0; j<k; j++) {
        scanf("%s %d %d", op, &u, &v);      //cin >> a >> u >> v;
        //a = string(op);
        // a = op;
         T= (op[0]=='a'); //first letter of "ans" is 'a'
        // T= (a[0]=='a'); //first letter of "ans" is 'a'
         L += T;
         Query[j][0]=T;
        Query[j][1]=u;
        Query[j][2]=v;
      ...
      for (int i=1; i<=n; i++) make_one_element_set(i);
       ...
       for(int j=k-1; j>=0; j--){
         u = Query[j][1];
         v = Query[j][2];
          if (Query[j][0] == 0) union_sets_of(u, v);
          else {
