...
*/
    void cravePath(int startX, int startY, gType g) {
        queue<Coord> q;
             ...
             int y = current.second;
              ...
              random_shuffle(begin(Dir), end(Dir), g);
               ...
               for (int i = 0; i < 4; ++i) {
                   ...
                   if (newX >= 0 && newX <= n-1  && newY >= 0 && newY <= n-1  && maze[newX][newY] == -1) {
                       maze[newX][newY] = 0;
    ...
    void generateMaze() {
        resizeAndInitializeMaze(n, n);
         ...
         cout << " Intial point for Maze generation: ("<<startX<<","<<startY<<")"<<endl;
         //maze[startX][startY] = 0;
         
          cravePath(startX, startY, g);
